...
output:
  html_document: 
    keep_md: yes
subtitle: |
  | Johns Hopkins University + Coursera Data Science Specialisation
...
Taking each daily total of steps, the mean of all totals is roughly `daily_mean:` `r daily_mean`.
Taking the same set of daily totals of steps, the median of these figures is `daily_median:` `r daily_median`.
...
The time period with the maximum average number of steps is `r maxtime`.
...
Answer to part 1: Out of a total of `r num_total` rows, `r num_na` have 'NA' as the step values.
...
Answer to part 4b: The new mean is `r full_mean` and the new median is `r full_median`.
As a reminder, the original (non-NA) mean was `r daily_mean` and the median was `r daily_median`.
